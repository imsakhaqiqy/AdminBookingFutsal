...
defined('BASEPATH')OR exit('No direct script access allowed ');
    class M_Deskripsi_Lapangan extends CI_Model{

        function selectAll(){
            $query = $this->db->query("SELECT * FROM  f_deskripsi_lapangan ORDER BY nomor ASC");
            return $query->result();
        }

		function count_data(){
			$query = $this->db->query("SELECT * FROM f_deskripsi_lapangan");
